...
def animate_double_pendulum(
    ...
    save_path : str | Path, optional
        If None, saves to '<project_root>/results/double_pendulum_animation.mp4'.
    show : bool
    ...
    T = x_traj.shape[0]
    assert x_traj.shape[1] == 4, "DoublePendulumModel assumed to have 4 states."
    ...
    fig, (ax_pend, ax_torque) = plt.subplots(
        1, 2, figsize=(7, 3.2), gridspec_kw={"wspace": 0.45}
    )
    ...
    # Rods + bobs
    rod1, = ax_pend.plot([], [], "o-", lw=2)   # pivot -> mass1
    rod2, = ax_pend.plot([], [], "o-", lw=2)   # mass1 -> mass2
    ...
    def update(frame_idx: int):
        # positions for this animation frame
        x1b, y1b = x1[frame_idx], y1[frame_idx]
        x2b, y2b = x2[frame_idx], y2[frame_idx]

        rod1.set_data([0.0, x1b], [0.0, y1b])
        rod2.set_data([x1b, x2b], [y1b, y2b])

        # torque (defined between samples)
        if frame_idx < T_anim - 1:
            uu = float(u_anim[frame_idx])
        else:
            uu = 0.0
    ...
    if save_path is None:
        ...
        save_path = results_dir / "double_pendulum_animation.mp4"
