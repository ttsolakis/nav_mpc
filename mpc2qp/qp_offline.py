...
class QPStructures:
    ...
    # Collision upper-bound indices into u (optional)
    idx_Ucol: np.ndarray | None     # (N, nc_col)
...
def build_qp(
    ...
    if nc_col > 0:
        ...
        eps_norm = float(collision.eps_norm)
    ...
    for k in range(N):
        ...
        # System block (nc_sys): placeholders
        ...
        # Collision block (nc_col): placeholders, applied to FUTURE state x_{k+1}
        if nc_col > 0:
            ...
            for j in range(nc_col):
                ...
                rows.append(r)
                cols.append(col_xcol + ix)
                data.append(0.0)
                rows.append(r)
                cols.append(col_xcol + iy)
                data.append(0.0)
    ...
    # inequality bounds: -inf <= (...) <= rhs (system rhs overwritten online)
    ...
    if nc_col > 0:
        ...
        idx_Cxy = np.empty((N, nc_col, 2), dtype=np.int64)
    ...
    if nc_col > 0:
        ...
        for k in range(N):
            ...
            for j in range(nc_col):
                r = ineq_row0 + nc_sys + j
                px = _csc_position(A_init, r, col_xcol + ix)
                py = _csc_position(A_init, r, col_xcol + iy)
                idx_Cx[k, j] = px
                idx_Cy[k, j] = py
                idx_Cxy[k, j, 0] = px
                idx_Cxy[k, j, 1] = py
    ...
    # -----------------------------------------------------------------------------
    # Collision upper-bound indices (into u vector)
    # -----------------------------------------------------------------------------
    idx_Ucol = None
    if nc_col > 0:
        idx_Ucol = np.empty((N, nc_col), dtype=np.int64)
        for k in range(N):
            base = n_eq + k * nc + nc_sys
            for j in range(nc_col):
                idx_Ucol[k, j] = base + j
